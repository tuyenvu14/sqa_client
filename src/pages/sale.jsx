...
export default function Sale() {
  ...
  const initPage = () => {
    callApi("sale_bill", "GET", null).then((res) => {
      if (res.status === 200) {
        var list = [];
        var x = 0;
        res.data.map((sale) => {
          if (converTimeToTimestamp(sale.date) < converTimeToTimestamp(endDate) && converTimeToTimestamp(sale.date) > converTimeToTimestamp(startDate)) {
            list.push({
              ...
              totalMoney: handleMoney(sale.itemInBillList),
              itemsList: sale.itemInBillList,
            });
            x = x + handleMoney(sale.itemInBillList);
          }
        });
        setDataRow(list);
        setMoney(x);
  ...
  const handleMoney = (x) => {
    var money = 0;
    x.map((y) => {
      money = money + y.quantity * y.outputPrice;
    });
    return money;
  };
  ...
  const onSave = () => {
    console.log(billAdd);
    callApi(`sale_bill`, "POST", billAdd);
    setX(1);
  };
  ...
  function Row(props) {
    ...
    const onDelete = (id) => {
      callApi(`sale_bill/${id}`, "DELETE", null);
      setX(-1);
    };
                  ...
                  <TableBody>
                    {row.itemsList.map((historyRow) => (
                      <TableRow key={historyRow.id}>
                        ...
                        <TableCell align="right">
                          {formatPrice(historyRow.outputPrice)}
                        </TableCell>
                        <TableCell align="right">
                          {formatPrice(
                            Math.round(
                              historyRow.quantity * historyRow.outputPrice * 100
                            ) / 100
                          )}
                        </TableCell>
                      </TableRow>
  ...
  return (
    <Layout>
      <Grid coitainer style={{ padding: "74px 240px 0px 0px" }}>
        <Grid container>
          <Grid item xs={12}>
            <h1 className="text-center">Sale Manager</h1>
          </Grid>
        </Grid>
                  ...
                  <from>
                    <div className="mb-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setBillAdd({
                            date: e.target.value,
                            customer: billAdd.customer,
                            itemInBillList: [
                              {
                                quantity: billAdd.itemInBillList[0].quantyti,
                                outputPrice: billAdd.itemInBillList[0].outputPrice,
                                item: billAdd.itemInBillList[0].item,
                              },
                            ],
                          });
                        }}
                    ...
                    <div className="mb-1">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        name="billAdd.customer.id"
                        value={billAdd.customer.id}
                        onChange={(e) => {
                          // setDate({
                          //   date: e.target.value,
                          // });
                    ...
                    <div className="mb-1">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        name="billAdd.itemInBillList[0].item.id"
                        value={billAdd.itemInBillList[0].item.id}
                        onChange={(e) => {
                          // setItemsId({
                          //   id: e.target.value,
                          //   // quantity: items.quantity,
                          //   // outputPrice: items.outputPrice
                          // });
                          setBillAdd({
                            date: billAdd.date,
                            customer: billAdd.customer,
                    ...
                    <div className="mb-1">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        name="billAdd.itemInBillList.quantity"
                        value={billAdd.itemInBillList[0].quantity}
                        onChange={(e) => {
                          // setItems({
                          //   // id: items.id,
                    ...
                    <div className="mb-1">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        name="billAdd.itemInBillList.outputPrice"
                        value={billAdd.itemInBillList[0].outputPrice}
                        onChange={(e) => {
                          // setItems({
                          //   // id: items.id,
                          //   quantity: items.quantity,
                          ...
                          setBillAdd({
                            ...
                            itemInBillList: [
                              {
                                quantity: billAdd.itemInBillList[0].quantity,
                                outputPrice: e.target.value,
                                item: billAdd.itemInBillList[0].item
                              },
                            ],
          ...
          <Table aria-label="collapsible table">
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{ minWidth: "220px" }}>Customer</TableCell>
                <TableCell align="right" style={{ minWidth: "160px" }}>
                  Total Price ($)
                </TableCell>
